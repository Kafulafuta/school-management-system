...
if(isset($_POST['submit']))
{
	...
	$sql="UPDATE markgrade SET grade_name='$grade_name',percent_from='$percent_from',percent_upto='$percent_upto' WHERE id='$id'";
	$query=mysqli_query($con,$sql);
	if($query)
	{
		echo "<p align='center' style='color: #00ff00;'>Grades Updated successfully</p>";
	}
	else
	{
		echo "<p align='center' style='color: #ff0000;'>Grades not Updated</p>";
	}
}
...
<?php
	$id=$_GET['id'];
	$sql="SELECT * FROM markgrade WHERE id=$id";
	$query=mysqli_query($con,$sql);
	while($result=mysqli_fetch_array($query))
	{
		$id=$result['id'];
		$grade_name=$result['grade_name'];
		$percent_from=$result['percent_from'];
		$percent_upto=$result['percent_upto'];
	}
	...
	<section class="site-section">
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-md-7">
            <div class="form-wrap">
              <h2 class="mb-5">Update Grade</h2>
              <form action="updatemarkgrade.php" method="post">
				  ...
				  <div class="row">
                    <div class="col-md-12 form-group">
                      ...
                      <input type="text" name="grade_name" class="form-control py-2 " value="<?php echo $grade_name;?>">
                    </div>
                  </div>
				  <div class="row">
                    <div class="col-md-12 form-group">
                      <label for="percent">Percent From(%)</label>
                      <input type="text" name="percent_from" class="form-control py-2 " value="<?php echo $percent_from;?>">
                    </div>
                  </div>
				   <div class="row">
                    <div class="col-md-12 form-group">
                      <label for="percent">Percent Upto(%)</label>
                      <input type="text" name="percent_upto" class="form-control py-2 " value="<?php echo $percent_upto;?>">
                    </div>
                  </div>
